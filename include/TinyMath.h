#ifndef TINYMATH_H_
#define TINYMATH_H_

#define fixed_point 0x04000000
#define fix(x) ((int64_t)x << 26)
#define fix32(x) ((int32_t)x << 26) 
#define fixed_point_div(x, y)  (x << 26)/y //TODO: make multiply and divide 
#define fixed_point_mult(x, y) ((x>>8)*(y>>8)) >> 10
#define INV_LOG2_E_Q1DOT31  UINT64_C(0x58b90bfc) // Inverse log base 2 of e

uint64_t abs(int64_t val);
int64_t expfix(int64_t val);
int64_t log2fix (int64_t x);
int64_t logfix (int64_t x);
int64_t powerfix(int64_t val, int64_t power);
int64_t power_simple(int64_t val, uint8_t p);
#endif /* INCFILE1_H_ */